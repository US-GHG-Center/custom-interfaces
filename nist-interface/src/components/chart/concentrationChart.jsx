...
const collectionItemURL = (collectionId, config) => {
  const baseUrl = config?.featuresApiUrl;
  const url = `${baseUrl}/collections/${collectionId}/items?is_max_height_data=True`;
  return url;
};
...
export class ConcentrationChart extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.props.selectedStationId !== prevProps.selectedStationId ||
      this.props.ghg !== prevProps.ghg
    ) {
  ...
  initializeChart = () => {
    ...
    this.chart.options.plugins.zoom.zoom.onZoomComplete = () => {
      ...
      this.chart.options.plugins.tooltip.enabled = this.isChartZoomed(
        this.chart,
        this.initialScaleLimits
      );
    };
  ...
  prepareChart = () => {
    let currentStationId = this.getChangedGHGStationId(
  ...
  fetchStationData = async (stationId) => {
    try {
      let url = collectionItemURL(stationId, this.props.config);
        ...
        <div id="chart-container" style={{ width: "100%", height: "100%" }}>
          <div id="chart-tools">
              ...
              <div id="chart-instructions-container">
                <div className="icon-and-instructions">
                  <FontAwesomeIcon
                    ...
                    // style={{margin: "12px"}}
                    onMouseEnter={() =>
                      this.setState({ showChartInstructions: true })
                    }
                    onMouseLeave={() =>
                      this.setState({ showChartInstructions: false })
                    }
                  />
                  {this.state.showChartInstructions && (
                    <div id="chart-instructions">
                      <p>
                        1. Click and drag, scroll or pinch on the chart to zoom
                        in.
                      </p>
                      <p>
                        2. Hover over data points when zoomed in to see the
                        values.
                      </p>
                      <p>
                        3. Click on the rectangle boxes on the side to toggle
                        chart.
                      </p>
                    </div>
                  )}
            ...
            <div id="chart-tools-right">
              {this.state.dataAccessLink && (
                <a
                  id="data-access-link"
                  href={this.state.dataAccessLink}
                  target="_blank"
                  rel="noreferrer"
                >
