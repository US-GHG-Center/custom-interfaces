...
async function processFeatures(features) {
  ...
  if (change_lookup) {
    fs.writeFileSync(
      "./lookups/plume_id_location.json",
      JSON.stringify(lookup_location, null, 2)
      );
...
main()
// async function create_lookup() {
  // const methane_stac_data = await (
  //   await fetch("http://localhost:8000/data/combined_plume_metadata.json")
  //   ).json();
  // // Write the data to a file
  ...
  // // Remove "style" field from each feature
  // methane_stac_data.features.forEach(feature => {
  //   let plume_id = feature.properties["Plume ID"];
    
  //   lookup_location[plume_id] = feature.properties.Location;
...
//   console.log(lookup_location['CH4_PlumeComplex-2296']);
